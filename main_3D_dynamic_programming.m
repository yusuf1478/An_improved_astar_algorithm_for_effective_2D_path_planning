...
% optimal path with dynamic programmin on undirectional map
[parent, Route]=dynamicpathplanning(undirectedGraph,nodelocation,exundnodIndex,snodeund,enodeund);
if ~isempty(Route)
    route=nodelocation(Route,:);
    cost=pathcost2(route);
    close all;
    drawRoute2('Dynamic Programming',snodeund,enodeund,nodelocation,exundnodIndex,unedges,Route,cost,1);
end
